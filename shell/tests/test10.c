...
main(void)
{
  struct pipeline* my_pipeline = pipeline_build("./a.exe >myoutputfile < myinputfile");
	...
	TEST_ASSERT(strcmp("./a.exe", my_pipeline->commands->command_args[0]) == 0);
	TEST_ASSERT(my_pipeline->commands->command_args[1] == NULL);

	// Test the redirect state
	TEST_ASSERT(strcmp("myoutputfile", my_pipeline->commands->redirect_out_path) == 0);
	TEST_ASSERT(strcmp("myinputfile", my_pipeline->commands->redirect_in_path) == 0);
