...
main(void)
{
	...
	TEST_ASSERT(strcmp("ab", my_pipeline->commands->command_args[0]) == 0);
	TEST_ASSERT(strcmp("cd", my_pipeline->commands->command_args[1]) == 0);	
	TEST_ASSERT(strcmp("e", my_pipeline->commands->command_args[2]) == 0);		
       	TEST_ASSERT(my_pipeline->commands->command_args[3] == NULL);

	// Test the redirect state
	TEST_ASSERT(strcmp("fg", my_pipeline->commands->redirect_in_path) == 0);
