...
main(void)
{
  struct pipeline* my_pipeline = pipeline_build("command > outputfile\n");
	...
	TEST_ASSERT(strcmp("command", my_pipeline->commands->command_args[0]) == 0);
	TEST_ASSERT(my_pipeline->commands->command_args[1] == NULL);

	// Test the redirect state
	TEST_ASSERT(strcmp(my_pipeline->commands->redirect_out_path, "outputfile") == 0);
