...
main(void)
{
	...
	TEST_ASSERT(strcmp("c", my_pipeline->commands->command_args[1]) == 0);	
	TEST_ASSERT(my_pipeline->commands->command_args[2] == NULL);

	// Test the redirect state
	TEST_ASSERT(strcmp("ab",my_pipeline->commands->redirect_in_path) == 0);
	TEST_ASSERT(strcmp("b",my_pipeline->commands->redirect_out_path) == 0);	
