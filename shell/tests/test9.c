...
main(void)
{
	struct pipeline* my_pipeline = pipeline_build("ls -l > output.txt\n");
	...
	TEST_ASSERT(strcmp("-l", my_pipeline->commands->command_args[1]) == 0);	
	TEST_ASSERT(my_pipeline->commands->command_args[2] == NULL);

	// Test the redirect state
	TEST_ASSERT(strcmp("output.txt", my_pipeline->commands->redirect_out_path) == 0);
